"""Wrapper: delegate to async implementation inspect_bill_async.py
...
ASYNC_PATH = os.path.join(HERE, 'inspect_bill_async.py')

if not os.path.exists(ASYNC_PATH):
    print('Missing async implementation:', ASYNC_PATH)
    sys.exit(1)

spec = util.spec_from_file_location('inspect_bill_async', ASYNC_PATH)
